...
public class Calculation2POIUtil {
    ...
    // 2个POI求坐标
    public static StandardLocationInfo getLocaionBy2Points(double startAngle, Map<String,TextDetectionAndPoi> locationInfoMap, Map<String,StandardLocationInfo> standardLocationInfoMap) {
        //通过map转换得到需要的POI及对应角度
        //String point1, double angle1, String point2,double angle2
        List<String> POINameList=new ArrayList<>();
        for(String POIName:locationInfoMap.keySet())
        {
            POINameList.add(POIName);
        }
        String point1=POINameList.get(0);
        String point2=POINameList.get(1);
        double angle1=locationInfoMap.get(point1).ori_angle;
        double angle2=locationInfoMap.get(point2).ori_angle;
        // 构建直角坐标系的4个坐标轴角度
        double[] baseAngles = new double[4];
        baseAngles[0] = startAngle;
        for (int i = 1; i < 4; i++) {
    ...
    // 1个POI求坐标
//	public StandardLocationInfo getLocaionByOnePoints(double startAngle, String point1, double angle1, String point2,
//			double angle2, double walkAngle, Double walkDistance, boolean isClockwise) {
//		if (isClockwise) {
//			// 构建直角坐标系的4个坐标轴角度
//			double[] baseAngles = new double[4];
//			baseAngles[0] = startAngle;
...
//			int quadrant2 = getQuadrant(baseAngles, angle2);
//			//行走直线的所属象限
//			int walkQuadrant = getQuadrant(baseAngles, walkAngle);
//
//			// 已知象限的情况下计算斜率
//			double slope1 = getSlope(baseAngles, quadrant1, angle1);
//			double slope2 = getSlope(baseAngles, quadrant2, angle2);
//			//行走直线的斜率
//			double walkSlope = getSlope(baseAngles, walkQuadrant, walkAngle);
//
//			// 计算截距
//			double intercept1 = getIntercept(standardLocationInfoMap.get(point1), slope1);
//			double intercept2 = getIntercept(standardLocationInfoMap.get(point2), slope2);
...
//			System.out.println("quarant1:" + quadrant1 + ",slope1:" + slope1 + ",intercept1:" + intercept1);
//			System.out.println("quarant2:" + quadrant2 + ",slope2:" + slope2 + ",intercept2:" + intercept2);
//			System.out.println("walkQuarant:" + walkQuadrant + ",walkSlope:" + walkSlope);
//
//
//			return null;
//		} else {
